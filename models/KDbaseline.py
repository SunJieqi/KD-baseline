...
class KD_baseline(nn.Module):
    ...
    def __init__(self, way=None, shots=None, resnet=False, is_pretraining=False, num_cat=None):
        ...
        self.resolution = 25
        self.lam = nn.Parameter(torch.zeros(1), requires_grad=not is_pretraining)
    ...
    def get_feature_map(self, inp):
        ...
        return feature_map.reshape(batch_size, self.d, -1).permute(0, 2, 1).contiguous()
    ...
    def get_neg_l2_dist(self, inp, way, shot, query_shot, lmd=0.1, return_support=False):
        ...
        alpha = self.r[0]
        beta = self.r[1]
        fea = self.get_feature_map(inp) 
        l, neg_l2_dist, support = self.distance_transform(fea, alpha, beta, d, lmd, query_shot, resolution, shot,
                                                          way)
    ...
    def distance_transform(self, fea, alpha, beta, d, lmd, query_shot, resolution, shot, way):
        support = fea[:way * shot].reshape(way, shot * resolution, d)
        query = fea[way * shot:].reshape(way * query_shot * resolution, d)
        query_ = fea[way * shot:].reshape(way * query_shot, resolution, d).mean(1).cpu()
        ...
        W = create_affinity(query_, knn=5)
        l = bound_update(unary, W, bound_lambda=lmd)
        l = torch.from_numpy(l).cuda()
        return l, neg_l2_dist, support

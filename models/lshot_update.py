...
def entropy_energy(Y, unary, kernel, bound_lambda, batch=True):
    tot_size = Y.shape[0]
    pairwise = kernel.dot(Y)
    if batch == False:
        temp = (unary * Y) + (-bound_lambda * pairwise * Y)
        E = (Y * np.log(np.maximum(Y, 1e-20)) + temp).sum()  # 10
    else:
        batch_size = 256
...
def bound_update(unary, kernel, bound_lambda, bound_iteration=20, batch=False):
    ...
    for i in range(bound_iteration):
        ...
        E = entropy_energy(Y, unary, kernel, bound_lambda, batch=True)
        E_list.append(E)

...
def default_train(train_loader, model, optimizer, writer, iter_counter):
    ...
    for i, (inp, _) in enumerate(train_loader):
        ...
        _, max_index1 = torch.max(log_prediction, 1)
        acc1 = 100 * torch.sum(torch.eq(max_index1, target)).item() / query_shot / way

        #_, max_index = torch.max(l, 1)

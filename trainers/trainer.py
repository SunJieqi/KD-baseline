...
def train_parser():
    ...
    parser.add_argument("--opt", default='sgd', help="optimizer", choices=['adam', 'sgd'])
    parser.add_argument("--lr", default=1e-1, help="initial learning rate", type=float)
    parser.add_argument("--gamma", default=1e-1, help="learning rate cut scalar", type=float)
    parser.add_argument("--epoch", default=400, help="number of epochs before lr is cut by gamma", type=int)
    parser.add_argument("--stage", default=2, help="number lr stages", type=int)
    parser.add_argument("--weight_decay", default=5e-4, help="weight decay for optimizer", type=float)
    ...
    parser.add_argument("--batch_size", help="batch size used during pre-training", type=int, default=256)
    parser.add_argument('--decay_epoch', nargs='+', help='epochs that cut lr', type=int)
    parser.add_argument("--pre", help="whether use pre-resized 84x84 images for val and test",
                        action="store_true")
    parser.add_argument("--no_val", help="don't use validation set, just save model at final timestep",
                        action="store_true")
    parser.add_argument("--train_way", default=20, help="training way", type=int)
    parser.add_argument("--test_way", help="test way", type=int, default=5)
    parser.add_argument("--train_shot", default=5,
                        help="number of support images per class for meta-training and meta-testing during validation",
                        type=int)
    parser.add_argument("--test_shot", nargs='+', default=[1, 5],
                        help="number of support images per class for meta-testing during final test", type=int)
    ...
    parser.add_argument("--test_query_shot", help="number of query images per class during meta-testing", type=int,
                        default=16)
    parser.add_argument("--train_transform_type", default=0, help="size transformation type during training", type=int)
    parser.add_argument("--test_transform_type", default=0, help="size transformation type during inference", type=int)
...
class Train_Manager:
    ...
    def train(self, model):
        ...
        if os.path.exists(args.resume):
            ...
            # checkpoint = model.load_state_dict(torch.load(args.resume, map_location=util.get_device_map("cuda:0")), strict=True)
            ...
            logger.info(
                f"finish resuming model from {args.resume}, epoch {checkpoint['best_epoch']}, train_acc: {checkpoint['best_val_acc']:.2f}, "
            )

        logger.info("start training!")

        for e in tqdm(range(start_epoch, total_epoch)):
            ...
            if (e + 1) % args.val_epoch == 0:
                ...
                model.eval()
                with torch.no_grad():
                    mean1, interval1, mean2, interval2 = meta_test(data_path=self.pm.val,
                                                                   model=model,
                                                                   way=test_way,
                                                                   shot=val_shot,
                                                                   pre=args.pre,
                    ...
                    writer.add_scalar('val_%d-way-%d-shot_acc1' % (test_way, val_shot), mean1, iter_counter)
                    writer.add_scalar('val_%d-way-%d-shot_acc2' % (test_way, val_shot), mean2, iter_counter)
                logger.info('val_%d-way-%d-shot_acc1: %.3f\t%.3f' % (test_way, val_shot, mean1, interval1))
                logger.info('val_%d-way-%d-shot_acc2: %.3f\t%.3f' % (test_way, val_shot, mean2, interval2))
                if mean1 > best_val_acc:
                    best_val_acc = mean1
                    best_epoch = e + 1
                    if not args.no_val:
    ...
    def evaluate(self, model):
        ...
        with torch.no_grad():
            ...
            checkpoint = torch.load(self.save_path, map_location=util.get_device_map(gpu=0))
            model.load_state_dict(checkpoint["model_state_dict"], strict=True)
            model.eval()

            for shot in [1, 5]:
                mean1, interval1, mean2, interval2 = meta_test(data_path=self.pm.test,
                                                               model=model,
                                                               way=args.test_way,
                                                               shot=shot,
                                                               pre=args.pre,
                                                               ...
                                                               query_shot=args.test_query_shot,
                                                               trial=2000)
                logger.info('%d-way-%d-shot acc1: %.2f\t%.2f' % (args.test_way, shot, mean1, interval1))
                logger.info('%d-way-%d-shot acc2: %.2f\t%.2f' % (args.test_way, shot, mean2, interval2))
